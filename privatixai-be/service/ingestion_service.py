...
from vectorstore.chroma_store import upsert_embeddings
...
def ingest_text_file(file_path: Path, file_id: str, set_status) -> None:
    """
    Synchronous ingestion pipeline. `set_status(stage, progress, error?)` updates status.
    """
    try:
        ...
        # Use BGE-m3 passage prefix for document chunks
        ...
        upsert_embeddings(embeddings=embeddings, metadatas=metadatas, ids=chunk_ids)
...
def ingest_file_any(file_path: Path, file_id: str, set_status) -> None:
    """Detect type and run appropriate ingestion. Supports text and audio for MVP."""
    ...
    if suffix in settings.SUPPORTED_AUDIO_FORMATS:
        try:
            ...
            # Use BGE-m3 passage prefix for transcribed chunks
            embeddings = embed_passages(chunk_texts)
            set_status(IngestionStage.UPSERTING, 70)
            emit_event("ingestion_stage", {"file_id": file_id, "stage": IngestionStage.UPSERTING, "chunks": len(chunk_ids)})
            upsert_embeddings(embeddings=embeddings, metadatas=metadatas, ids=chunk_ids)
